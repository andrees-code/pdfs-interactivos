...
`
  content =
    content.slice(0, loadPresentationsMatch) +
    logicSnippet +
    '\n' +
    content.slice(loadPresentationsMatch)

  // Inyectar en la carga
  const fetchMatch = content.indexOf(
    'presentations.value = await presentationService.getPresentationsByUserId(user.id);',
  )

...
export async function importPptxFull(file: File): Promise<FullImportResult> {
  ...
  const fetchWithTimeout = (url: string, timeout: number): Promise<Response> => {
    ...
    return fetch(url, { method: 'POST', body: buildForm(), signal: ctrl.signal }).finally(() =>
      clearTimeout(id),
    )
  }
  ...
  for (const [pageStr, pageData] of Object.entries(pages)) {
    ...
    if (mapped.length > 0) {
      ...
      console.log(
        `[usePptxFullImport] Page ${pageStr}: ${mapped.length} elementos mapeados. First element:`,
        {
          id: first.id,
          type: first.type,
...
function mapElementToEditor(el: unknown): Record<string, unknown> {
  ...
  const base = {
    ...
    animationType: asBoolean(source.isAnimated)
      ? asString(source.animationType, 'fade-in')
      : 'none',
    animationOrder: asBoolean(source.isAnimated) ? asNumber(source.animationOrder, 0) : 0,
    animationTrigger: asBoolean(source.isAnimated)
      ? asString(source.animationTrigger, 'onClick')
      : 'onClick',
  }
  ...
  switch (asString(source.type, 'shape')) {
    case 'text':
      const textElement = {
        ...
        animation: asBoolean(source.isAnimated)
          ? asString(source.animationType, 'fade-in')
          : 'none',
      }
